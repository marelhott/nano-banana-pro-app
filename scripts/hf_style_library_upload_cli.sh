...
# Upload local SD checkpoints + LoRA safetensors into a single HF model repo using `hf upload`.
#
# Usage:
#   HF_TOKEN=... bash scripts/hf_style_library_upload_cli.sh
#
# Optional env:
#   HF_OWNER=mulenmara
...
python3 - "$SD_SRC" "$LORA_SRC" >"$tmp/manifest.json" <<'PY'
import json
import os
import sys
...
hf upload "$TARGET_REPO_ID" "$tmp/manifest.json" "manifest.json" \
  --commit-message "Add manifest.json" >/dev/null

echo "==> Uploading SD checkpoints -> checkpoints/"
hf upload "$TARGET_REPO_ID" "$SD_SRC" "checkpoints" \
  --include "*.safetensors" \
  --exclude "._*" \
  --commit-message "Upload checkpoints" \
  --commit-description "Upload SD checkpoints from local library." \
  --no-quiet

echo "==> Uploading LoRAs -> loras/"
hf upload "$TARGET_REPO_ID" "$LORA_SRC" "loras" \
  --include "*.safetensors" \
  --exclude "._*" \
  --commit-message "Upload LoRAs" \
  --commit-description "Upload LoRA weights from local library." \
  --no-quiet
