...
export const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  const commonClasses = `relative group cursor-pointer transition-all duration-300 border rounded-md flex flex-col items-center justify-center ${isDragging
  ...
  return (
    <div
      className={`${commonClasses} h-32 w-full text-center p-4`}
      onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }}
      onDragLeave={() => setIsDragging(false)}
      onDrop={handleDrop}
    >
      <input type="file" accept="image/*" multiple onChange={(e) => e.target.files && processFiles(e.target.files)} className="absolute inset-0 opacity-0 cursor-pointer z-10" title="" />
      <div className="bg-monstera-300 text-ink p-3 rounded-md mb-2 shadow-xl shadow-monstera-300/20 transform group-hover:scale-105 transition-all">
      ...
      <h3 className="text-xs font-black text-ink uppercase tracking-[0.2em]">Nahrát</h3>
    </div>
  );
};
