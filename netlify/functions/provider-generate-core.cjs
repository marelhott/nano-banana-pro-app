...
async function generateGrokImage(payload, apiKey) {
  const response = await fetch('https://api.x.ai/v1/images/generations', {
    ...
    body: JSON.stringify({
      ...
      image_url: payload.images?.[0]?.data,
      n: 1,
  ...
  if (!imageB64) throw new Error('No image data returned from Grok API');
  return { imageBase64: `data:image/png;base64,${imageB64}`, modelId: 'grok-imagine-image' };
}
